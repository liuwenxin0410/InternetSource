...
Write_Command3(0x02);	//--set low column address
...
Write_Command3(0xAD);	//--DC-DC ON/OFF Mode Set 
Write_Command3(0x8B);	//--8A:DC-DC is disable,8B:DC-DC will be turned on when display on.(POR)
...
void pic13(void)
{
	...
	for(p=0;p<8;p++)
	{
		...
		Write_Command3(0xb0+p);//--set start page
			...
			for(i=0;i<16;i+=2)              //picture loop
			    { 
				  for(page=0;page<8;page++)   //page loop
				    { 
					  Write_Command3(0x22);//--set page1
	                  Write_Command3(0xb0+page);//--set start page
		              Write_Command3(0x07);//--set end page
					 ...
					 for(column=0;column<64;column++)   //column loop
					     ...
					     {
						     Write_Data3(test_tab[i]);
							 Write_Data3(test_tab[i+1]);
